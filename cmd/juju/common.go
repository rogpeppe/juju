...
// If it's a local charm URL, the local charm repository at
...
// the charm.
func resolveCharmURL(curlStr string, repoPath string, conf *config.Config) (*charm.URL, charmrepo.Interface, error) {
	...
	if ref.Schema == "local" && ref.Series == "" {
		...
		return nil, nil, fmt.Errorf("cannot resolve series for charm: %q", ref)
	}
...
var charmStoreParams = func() (charmrepo.NewCharmStoreParams, error) {
	return newCharmStoreParams, nil
}
